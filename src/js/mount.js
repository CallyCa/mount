...
function makeMountDataStr(object) {
  ...
  for (let key in object) result.push(key + "=" + object[key]);
...
function checkArguments(target, fsType, options, dataStr, callback) {
  ...
  if (
    (options != null && options.constructor.name === "Object") ||
  ...
  if (typeof options !== "number" && options.constructor !== Array)
    throw new Error(
      "options must be an array or a number, not " + typeof options
    );

  // ensure that `dataStr` is a string or a literal object
  if (typeof dataStr !== "string" && dataStr.constructor !== Object)
    throw new Error(
      "dataStr must be a string or an object, not " + typeof dataStr
    );

  if (options instanceof Array) options = makeMountFlags(options);
...
function fsList(data) {
  let supported = data.split("\n").filter(removeNoDev).map(trim);

  return module.exports.overrideOrder
    .concat(supported)
    .filter(removeDuplicates);
}

function _mount(target, fsType, options, dataStr, cb) {
  let argv = checkArguments(target, fsType, options, dataStr, cb);
  ...
  else _binding.mount.apply(_binding, argv);
}

function _mountSync(target, fsType, options, dataStr) {
  let argv = checkArguments(target, fsType, options, dataStr);

  argv.length = 5;

  if (argv[2] === "auto" && !(argv[3] & module.exports.MS_MOVE)) {
    let filesystems = fs.readFileSync("/proc/filesystems", "utf8");

    filesystems = fsList(filesystems);

    for (argv[2] in filesystems)
      if (_binding.mountSync.apply(_binding, argv)) return true;
...
function _umountSync(target) {
  ...
  if (typeof target !== "string") throw new Error("Invalid arguments");
